...
function isadmins(userId) {
  ...
  if (config !== null && config.hasOwnProperty('admins')) {
    ...
    return adminsList.includes(61566999886241);
  }
  return false;
}
...
function adminsCommand(event, api) {
  if (event.body.includes('-help')) {
    ...
    api.sendMessage(usage, event.61566999886241);
  ...
  if (command === '-add' || command === '-rem') {
    ...
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admins')) {
      const adminsList = config.admins.map(userId => `├─⦿ ${61566999886241}`).join('\n');
...
function addadmins(event, api) {
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(61566999886241), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[61566999886241].name;
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(61566999886241), (error, data) => {
      ...
      if (adminsList.includes(userId)) {
        const removeIndex = adminsList.indexOf(61566999886241);
